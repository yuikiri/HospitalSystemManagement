...
<c:if test="${not empty sessionScope.errorMessage}">
    ...
    <c:remove var="errorMessage" scope="session" /> </c:if>
...
<div class="row fade-in">
    <div class="col-md-4 mb-4">
        <div class="profile-card p-4 text-center h-100 d-flex flex-column justify-content-center">
            ...
            <p class="text-muted mb-3"><i class="fas fa-id-badge me-2"></i>Mã BN: #BN-${patient.id}</p>
        ...
        <div class="profile-card p-5 h-100">
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    ...
                    <p class="info-value">${patient.dob}</p>
                </div>
                <div class="col-sm-6">
                ...
                <form action="UpdatePatientController" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="date" name="dob" class="form-control rounded-3" value="${patient.dob}" required>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-12">
                            <label class="form-label text-muted fw-semibold">Email tài khoản</label>
                            <div class="input-group">
                                <span class="input-group-text bg-light text-muted"><i class="fas fa-lock"></i></span>
                                <input type="email" class="form-control bg-light text-muted" value="${sessionScope.user.email}" readonly>
                                <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#changeEmailModal">
                                    Thay đổi Email
                                </button>
                            </div>
                            <small class="text-muted">Email dùng để đăng nhập nên cần xác thực khi thay đổi.</small>
                        </div>
...
<div class="modal fade" id="changeEmailModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0" style="border-radius: 20px;">
            <div class="modal-header border-bottom-0 pb-0 mt-3 px-4">
                <h5 class="modal-title fw-bold text-primary"><i class="fas fa-envelope-open-text me-2"></i> Xác thực thay đổi Email</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4">
                <form action="RequestChangeEmailController" method="POST">
                    <div class="mb-3">
                        <label class="form-label text-muted fw-semibold">Nhập Email mới</label>
                        <input type="email" name="newEmail" class="form-control rounded-3" placeholder="Nhập email mới của bạn..." required>
                    </div>
                    <div class="alert alert-warning border-0 rounded-3 small">
                        <i class="fas fa-info-circle me-1"></i> Sau khi bấm xác nhận, hệ thống sẽ gửi một <b>Mã OTP / Link xác nhận</b> vào Email mới này. Bạn cần xác thực để hoàn tất việc thay đổi.
                    </div>
                    <div class="text-end mt-3">
                        <button type="button" class="btn btn-light rounded-pill px-4 me-2" data-bs-dismiss="modal">Hủy</button>
                        <button type="submit" class="btn btn-primary rounded-pill px-4">Gửi yêu cầu xác thực</button>
                    </div>
                </form>
            </div>
        </div>
