...
<%
    ...
    if (activeType == null) activeType = "doctor"; // Mặc định mở tab Bác sĩ
...
<head>
    ...
    <style>
        ...
        .card { 
            ...
            box-shadow: var(--soft-shadow); background: #ffffff;
            transition: all 0.3s ease;
        }

        /* Tabs Styling (Đồng bộ ManageRooms) */
        .nav-tabs { gap: 10px; border-bottom: none; }
        .nav-tabs .nav-link { 
            font-weight: 600; border: none; color: var(--text-muted); 
            padding: 12px 24px; border-radius: 12px !important; 
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); cursor: pointer;
        }
        .nav-tabs .nav-link:hover { background: #f8fafc; color: var(--purple-main); }
        .nav-tabs .nav-link.active { 
        ...
        .search-box, .filter-select {
            ...
            background-color: #fff !important; padding: 12px 20px;
            font-size: 0.95rem; box-shadow: 0 2px 5px rgba(0,0,0,0.02);
            transition: all 0.3s ease;
        }
        .search-box:focus-within, .filter-select:focus {
        ...
        .btn-primary {
            ...
            padding: 12px 24px; font-weight: 600; transition: 0.3s;
        }
        ...
        .btn-purple:hover { color: white; transform: translateY(-2px); box-shadow: 0 8px 15px rgba(139, 92, 246, 0.25); }
        .btn-light {
            border-radius: 12px; background: #fff; color: var(--text-main); font-weight: 600;
            border: 1px solid #e2e8f0; padding: 12px 24px; transition: all 0.3s;
        }
        .btn-light:hover { background: #f8fafc; transform: translateY(-2px); box-shadow: 0 5px 10px rgba(0,0,0,0.05); }

        /* Timetable Specific Enhancements */
        .table-schedule th {
            font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px;
            font-weight: 700; color: var(--text-muted); border-bottom: none !important;
            text-align: center; padding: 1.25rem 1rem;
        }
        .shift-time-col {
            background: #f8fafc; font-weight: 700; text-align: center; 
            border-radius: 14px; vertical-align: middle; color: var(--text-main);
            border: 1px solid #f1f5f9;
        }
        .shift-cell {
            background: #ffffff; border: 1px dashed #e2e8f0; border-radius: 14px;
            height: 100px; text-align: center; vertical-align: middle; cursor: pointer; transition: all 0.2s;
        }
        .shift-cell:hover { background: #f8fafc; border-color: var(--purple-main); transform: scale(1.02); z-index: 10; position: relative;}
        
        .shift-cell.has-shift { background: #f0fdf4; border: 1px solid #bbf7d0; border-left: 4px solid #10b981; }
        .shift-cell.has-shift:hover { background: #dcfce7; }
        ...
        .modal-footer { border-top: 1px solid #f1f5f9; padding: 1.25rem 2rem; background: #fff; }
        .form-label { font-weight: 600; color: var(--text-main); margin-bottom: 8px; font-size: 0.9rem; }
        .form-control { border-radius: 12px; padding: 12px 16px; border: 1px solid #e0e5f2; font-size: 0.95rem; }
        .form-control:focus { border-color: var(--purple-main); box-shadow: 0 0 0 4px rgba(139, 92, 246, 0.1); }
    </style>
</head>
<body>
...
<div class="container-fluid admin-container py-4 px-xl-5">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div>
            <h3 class="fw-bold mb-1" style="color: var(--text-main);">Quản lý Ca Trực</h3>
            <p class="text-muted mb-0"><i class="fa fa-calendar-days me-2"></i>Tra cứu và phân bổ thời khóa biểu nhân sự</p>
        </div>
    ...
    <div id="alertContainer">
        <% if(request.getParameter("error") != null) { %>
            <div class="alert alert-danger alert-dismissible fade show shadow-sm" style="border-radius: 12px; border: none;" role="alert">
                <i class="fa fa-exclamation-triangle me-2"></i><strong>Thất bại!</strong> Bị trùng lịch hoặc ID phòng không tồn tại.
                <button type="button" class="btn-close shadow-none" data-bs-dismiss="alert"></button>
            </div>
        <% } %>
    </div>
    ...
    <div class="mb-4">
        <ul class="nav nav-tabs" id="shiftTabs" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link <%= "doctor".equals(activeType) ? "active" : "" %>" data-bs-toggle="tab" data-bs-target="#doctor-tab">
                    <i class="fas fa-user-md me-2"></i>Lịch Bác Sĩ
                </button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link <%= "staff".equals(activeType) ? "active" : "" %>" data-bs-toggle="tab" data-bs-target="#staff-tab">
                    <i class="fas fa-user-nurse me-2"></i>Lịch Nhân Viên
                </button>
            </li>
        </ul>
    </div>
        ...
        <div class="tab-pane fade <%= "doctor".equals(activeType) ? "show active" : "" %>" id="doctor-tab">
            <div class="card p-4 mb-4">
                <form action="AdminController" method="get" class="row g-3 align-items-center">
                    ...
                    <div class="col-md-5">
                        <div class="input-group search-box">
                            <span class="input-group-text border-0 pe-2"><i class="fa fa-envelope text-muted"></i></span>
                            <input type="email" name="docEmail" class="form-control border-0 shadow-none p-0" placeholder="Nhập email bác sĩ..." value="<%= docEmail != null ? docEmail : "" %>" required>
                        </div>
                    </div>
                    
                    <div class="col-md-4">
                        <select name="weekOffset" class="form-select filter-select shadow-none fw-semibold">
                            <option value="0" <%= currentWeekOffset == 0 ? "selected" : "" %>>📅 Tuần hiện tại</option>
                            <option value="1" <%= currentWeekOffset == 1 ? "selected" : "" %>>📅 Tuần sau (+1 tuần)</option>
                            <option value="2" <%= currentWeekOffset == 2 ? "selected" : "" %>>📅 2 tuần tới (+2 tuần)</option>
                            <option value="3" <%= currentWeekOffset == 3 ? "selected" : "" %>>📅 3 tuần tới (+3 tuần)</option>
                            <option value="4" <%= currentWeekOffset == 4 ? "selected" : "" %>>📅 4 tuần tới (+4 tuần)</option>
                        </select>
                    </div>
                    
                    <div class="col-md-3">
                        <button type="submit" class="btn btn-primary btn-purple w-100"><i class="fas fa-search me-2"></i>Tải lịch trình</button>
                    </div>
                </form>
            </div>
            ...
            <div class="card p-4">
                <% if (docSchedule != null) { %>
                    <div class="table-responsive">
                        <table class="table table-schedule w-100" style="border-collapse: separate; border-spacing: 6px;">
                            <thead>
                                <tr>
                                    <th style="width: 9%;"><i class="far fa-clock"></i> Ca</th>
                                    <% for(String day : daysOfWeek) { %><th style="width: 13%;"><%=day%></th><% } %>
                                </tr>
                            </thead>
                            <tbody>
                                <% for(int r = 0; r < 8; r++) { %>
                                <tr>
                                    <td class="shift-time-col">
                                        Ca <%=r+1%><br><span class="text-muted fw-normal" style="font-size: 0.8rem;"><%=shiftTimes[r]%></span>
                                    </td>
                                    <% for(int c = 0; c < 7; c++) { 
                                        ShiftDTO shift = docSchedule[r][c];
                                        boolean hasShift = (shift != null);
                                    %>
                                        <td class="shift-cell <%= hasShift ? "has-shift" : "" %>"
                                            onclick="openShiftModal('doctor', '<%=docEmail%>', <%=c+2%>, <%=r+1%>, <%=currentWeekOffset%>, '<%=hasShift ? shift.getRoomId() : ""%>', '<%=hasShift && shift.getNote() != null ? shift.getNote() : ""%>')">
                                            <% if (hasShift) { %>
                                                <div class="fw-bold text-success mb-1" style="font-size: 0.85rem;"><%=shift.getDepartmentName()%></div>
                                                <div class="small" style="color: var(--text-main);">Phòng: <strong><%=shift.getRoomNumber()%></strong></div>
                                                <div class="small text-muted mt-1"><i class="fa fa-tag me-1" style="font-size: 0.7rem;"></i><%=shift.getNote()%></div>
                                            <% } else { %>
                                                <span class="text-muted opacity-25" style="font-size: 0.85rem;"><i class="fa fa-plus d-block mb-1 fs-5"></i> Trống</span>
                                            <% } %>
                                        </td>
                                    <% } %>
                                </tr>
                ...
                <% } else { %>
                    <div class="empty-state">
                        ...
                        <h5 class="fw-bold mt-3">Chưa tải dữ liệu lịch trình</h5>
                        <p>Vui lòng nhập Email của Bác sĩ và nhấn "Tải lịch trình" để phân bổ ca trực.</p>
                    </div>
                <% } %>
            </div>
        </div>
        ...
        <div class="tab-pane fade <%= "staff".equals(activeType) ? "show active" : "" %>" id="staff-tab">
            <div class="card p-4 mb-4">
                <form action="AdminController" method="get" class="row g-3 align-items-center">
                    ...
                    <div class="col-md-5">
                        <div class="input-group search-box" style="border-color: #e2e8f0;">
                            <span class="input-group-text border-0 pe-2"><i class="fa fa-envelope text-muted"></i></span>
                            <input type="email" name="staffEmail" class="form-control border-0 shadow-none p-0" placeholder="Nhập email nhân viên..." value="<%= staffEmail != null ? staffEmail : "" %>" required>
                        </div>
                    </div>
                    ...
                    <div class="col-md-3">
                        <button type="submit" class="btn btn-primary w-100" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);">
                            <i class="fas fa-search me-2"></i>Tải lịch trình
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div class="card p-4">
                        ...
                        <table class="table table-schedule w-100" style="border-collapse: separate; border-spacing: 6px;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <% for(int c = 0; c < 7; c++) { 
                                        ShiftDTO shift = staffSchedule[r][c];
                                        boolean hasShift = (shift != null);
                                    %>
                                        <td class="shift-cell <%= hasShift ? "has-shift" : "" %>"
                                            onclick="openShiftModal('staff', '<%=staffEmail%>', <%=c+2%>, <%=r+1%>, <%=currentWeekOffset%>, '<%=hasShift ? shift.getRoomId() : ""%>', '<%=hasShift && shift.getNote() != null ? shift.getNote() : ""%>')">
                                            <% if (hasShift) { %>
                                                <div class="fw-bold text-success mb-1" style="font-size: 0.85rem;"><%=shift.getDepartmentName()%></div>
                                                <div class="small" style="color: var(--text-main);">Phòng: <strong><%=shift.getRoomNumber()%></strong></div>
                                                <div class="small text-muted mt-1"><i class="fa fa-tag me-1" style="font-size: 0.7rem;"></i><%=shift.getNote()%></div>
                ...
                <% } else { %>
                    <div class="empty-state">
                        <i class="fa fa-calendar-xmark"></i>
                        <h5 class="fw-bold mt-3">Chưa tải dữ liệu lịch trình</h5>
                        <p>Vui lòng nhập Email của Nhân viên và nhấn "Tải lịch trình" để phân bổ ca trực.</p>
                    </div>
                <% } %>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="AdminController?action=updateShift" method="POST">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fa fa-calendar-plus me-2 text-primary"></i>Phân bổ ca trực</h5>
                    <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-light border border-info text-info mb-4 rounded-3 d-flex align-items-center">
                        <i class="fas fa-info-circle fs-3 me-3"></i>
                        <div>
                            <strong id="modalInfoText" class="d-block">Thứ 2 - Ca 1 (06:00 - 08:00)</strong>
                            <small class="text-muted">Gán cho: <span id="modalTargetLabel" class="fw-bold text-dark">...</span></small>
                        </div>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Nhập ID Phòng <span class="text-danger">*</span></label>
                        <div class="input-group search-box p-0">
                            <span class="input-group-text border-0 text-muted"><i class="fa fa-hashtag"></i></span>
                            <input type="number" name="roomId" id="modalRoomId" class="form-control border-0 shadow-none bg-transparent" placeholder="Ví dụ: 1, 2, 5..." required>
                        </div>
                    </div>
                    <div class="mb-2">
                        <label class="form-label">Vai trò / Nhiệm vụ cụ thể <span class="text-danger">*</span></label>
                        <div class="input-group search-box p-0">
                            <span class="input-group-text border-0 text-muted"><i class="fa fa-clipboard-list"></i></span>
                            <input type="text" name="roleNote" id="modalRoleNote" class="form-control border-0 shadow-none bg-transparent" placeholder="VD: Trực chính, Vệ sinh khu Nhi..." required>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary btn-purple px-4">Lưu Phân Công</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    function openShiftModal(type, email, day, shift, week, roomId, note) {
        ...
        const shiftTimes = {
            1: "06:00 - 08:00", 2: "08:00 - 10:00", 3: "10:00 - 12:00", 4: "12:00 - 14:00",
            5: "14:00 - 16:00", 6: "16:00 - 18:00", 7: "18:00 - 20:00", 8: "20:00 - 22:00"
        };
        ...
        document.getElementById('modalInfoText').innerText = dayNames[day] + " - Ca " + shift + " (" + shiftTimes[shift] + ")";
        document.getElementById('modalTargetLabel').innerText = email;

        new bootstrap.Modal(document.getElementById('editShiftModal')).show();
    }

    // Tự động ẩn Alert sau 3 giây (Đồng bộ với manageRooms)
    document.addEventListener("DOMContentLoaded", function() {
        const alerts = document.querySelectorAll('.alert-dismissible');
        alerts.forEach(function(alert) {
            setTimeout(function() {
